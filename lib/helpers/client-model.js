...
/**
 * @typedef {object} ClientDocument The projected client shape read from Mongo (`getBy` with the `code` and `webhookSubscriptions` fields)
...
/**
 * @typedef {object} CachedClient The in-memory shape, kept for reads
 * @property {string} code The Janis Client code
 * @property {Set<string>} [webhookSubscriptions] The client's synced webhook subscription keys, as a `Set` for O(1) lookup
 */
 ...
 *
 * Reads are cached in-memory per clientCode for 5 minutes (as a `Set`, for O(1) subscription lookup); writes are never cached.
 */
module.exports = class ClientModel {
	 ...
	 * @returns {Promise<Set<string>|undefined>} The subscription keys as a `Set`, or `undefined` if the client was never synced
	...
	static async getSubscriptions(clientCode) {
		...
		const [clientDocument] = await model.getBy('code', clientCode, { limit: 1, fields: ['code', 'webhookSubscriptions'] });
		...
		const client = {
			...clientDocument,
			webhookSubscriptions: clientDocument.webhookSubscriptions && new Set(clientDocument.webhookSubscriptions)
		};
	 ...
	 * @returns {CachedClient|undefined}
	 */
